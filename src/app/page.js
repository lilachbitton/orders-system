    ...
    const loadData = async () => {
      try {
        ...
        const customersResult = Papa.parse(customersText, {
          header: true,
          skipEmptyLines: true,
          dynamicTyping: true // חשוב: מבטיח שה-ID יהיה מספר
        });
        
        // שמירת הלקוחות עם השם וה-ID שלהם
        const validCustomers = customersResult.data
          .filter(row => row['שם הלקוח'] && row['מזהה מספר']) // מוודא שיש גם שם וגם ID
          .map(row => ({
            name: row['שם הלקוח']?.trim() || '', // טיפול במקרה של undefined
            id: row['מזהה מספר']
          }));
...
const totalUnits = orders.reduce((sum, order) => sum + order.quantity, 0);
  ...
  const submitOrder = async (orderDetails) => {
    try {
      // מציאת הלקוח המלא (כולל ID) לפי השם שנבחר
      const selectedCustomerData = customers.find(c => c.name === selectedCustomer);
      if (!selectedCustomerData) {
        throw new Error('לא נמצאו פרטי לקוח מלאים');
      }
      ...
      const customerSearchResponse = await fetch('https://api.yeshinvoice.co.il/api/v1/getAllCustomers', {
        ...
        body: JSON.stringify({
          "PageSize": 1000,
          "PageNumber": 1,
          "Search": selectedCustomerData.id.toString(), // חיפוש לפי ID
          "PortfolioID": 0,
          "orderby": {
            "column": "ID",
            "asc": "asc"
      ...
      const exactCustomer = customerData.ReturnValue.find(c => c.id === selectedCustomerData.id);
